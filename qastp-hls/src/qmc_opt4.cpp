...
/* Reduction */
template <int SIZE> void reductionFP4(fp_t fpBuffer[NPC]) {
    /* Force Inline */
#pragma HLS INLINE
    reductionFP4<SIZE / 2>(fpBuffer);
...
template <> void reductionFP4<2>(fp_t fpBuffer[NPC]) {
    /* Force Inline */
#pragma HLS INLINE
...
#if !UNROLL_TEST
template <int t>
void TrotterUnit4(const int nTrot, const int nSpin, const int ctlStep,
...
/* Force Inline */
#pragma HLS INLINE
    DuplicateTrotterUnits4<NTROT - 1>(nTrot, nSpin, ctlStep, iPre, j, startStep,
...
#pragma HLS INLINE
    ...
    TrotterUnit4<0>(nTrot, nSpin, ctlStep, iPre[0], j, startStep[0], endStep[0],
...
#pragma HLS INTERFACE axis register both port=trottersOut
...
#pragma HLS UNROLL
        ...
        DuplicateTrotterUnits4<MAX_NTROT>(
