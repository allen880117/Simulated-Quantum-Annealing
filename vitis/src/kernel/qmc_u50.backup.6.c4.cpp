...
void Run(const u32_t stage, const info_t info, const state_t state,
         spin_pack_u50_t trotters_local[NUM_SPIN / PACKET_SIZE],
         fp_pack_strm_t  j_stream[NUM_STREAM]) {
...
#pragma HLS UNROLL
            ...
            fp_pack_t tmp_pack = j_stream[strm_ofst].read();
...
class CacheUnit {
   private:
    fp_pack_t jcoup_local[NUM_TROT][NUM_SPIN / PACKET_SIZE / NUM_STREAM]
                         [NUM_STREAM];
...
#pragma HLS INLINE OFF
        ...
        // if (stage < NUM_SPIN) {
    READ_JCOUP:
        for (u32_t ofst = 0; ofst < NUM_SPIN / PACKET_SIZE / NUM_STREAM;
             ofst++) {
#pragma HLS PIPELINE
    ...
    void ShiftJcoupCache(fp_pack_strm_t j_stream[NUM_TROT][NUM_STREAM]) {
#pragma HLS INLINE OFF
    SHIFT_JCOUP:
        for (u32_t ofst = 0; ofst < NUM_SPIN / PACKET_SIZE / NUM_STREAM;
             ofst++) {
