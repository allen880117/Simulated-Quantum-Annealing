...
int main(int argc, char *argv[]) {
  ...
  for (int t = 0; t < nTrot; t++) {
    for (int i = 0; i < nSpin / PACKET_SIZE / NUM_STREAM; i++) {
      for (int k = 0; k < PACKET_SIZE * NUM_STREAM; k++) {
        trottersPack[t][i][k] = trotters[t][i * PACKET_SIZE * NUM_STREAM + k];
      }
    }
  }
#else
  ...
  for (int i = 0; i < iter; i++) {
...
#if !U50
    /* Execute */
    QuantumMonteCarlo(trottersPack, JcoupStream_0, h, Jperp, Beta, logRandNum);
#else
    QuantumMonteCarloU50(trottersPack, JcoupPack, h, Jperp, Beta, logRandNum);
#endif
#else
    ...
    for (int t = 0; t < nTrot; t++) {
      for (int i = 0; i < nSpin / PACKET_SIZE / NUM_STREAM; i++) {
        for (int k = 0; k < PACKET_SIZE * NUM_STREAM; k++) {
          trotters[t][i * PACKET_SIZE * NUM_STREAM + k] = trottersPack[t][i][k];
        }
      }
    }
#else
  ...
  /* Print Best Trotter */
  std::cout << "Run, Trotter: " << bestRun << " ," << bestTrotNum << std::endl;
  for (int i = 0; i < nSpin; i++) {
