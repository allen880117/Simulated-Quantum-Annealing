...
/* Reduction */
template <int SIZE> void reductionFP5S(fp_t fpBuffer[NPC]) {
    /* Force Inline */
#pragma HLS INLINE
    reductionFP5S<SIZE / 2>(fpBuffer);
...
template <> void reductionFP5S<2>(fp_t fpBuffer[NPC]) {
    /* Force Inline */
#pragma HLS INLINE
...
/* Trotter Unit */
template <int t>
void TrotterUnit5S(const int nTrot, const int nSpin, const int ctlStep,
                   ...
                   const int boardLen, const bool constraint[9 * 9]) {
...
#pragma HLS INLINE
    ...
    TrotterUnit5S<NTROT - 1>(
        ...
        JcoupLocal[NTROT - 1], logRandNumber[NTROT - 1], boardLen,
        constraint[NTROT - 1]);
};

template <>
void DuplicateTrotterUnits5S<1>(
    const int nTrot, const int nSpin, const int ctlStep,
    const int iPre[MAX_NTROT], const int j, const int startStep[MAX_NTROT],
    const int endStep[MAX_NTROT], spin_t trotters[MAX_NTROT][MAX_NSPIN],
    const spin_t up_trotter[MAX_NTROT], const spin_t down_trotter[MAX_NTROT],
...
#pragma HLS INLINE
    ...
    TrotterUnit5S<0>(nTrot, nSpin, ctlStep, iPre[0], j, startStep[0],
                     endStep[0], trotters[0], up_trotter[0], down_trotter[0],
                     dH[0], hNext_0, Beta, dHTunnel, JcoupLocal[0],
                     logRandNumber[0], boardLen, constraint[0]);
}
...
void QuantumMonteCarloOpt5S(const int nTrot, const int nSpin,
                            ...
                            const int          boardLen,
                            const bool         constraint[MAX_NTROT][9 * 9]) {
#pragma HLS INTERFACE s_axilite port=boardLen
#pragma HLS INTERFACE s_axilite port=constraint
    /* Interface */
...
#pragma HLS ARRAY_PARTITION variable = constraint complete dim = 1
...
#pragma HLS DEPENDENCE variable = constraint inter false
            /* Exit Condition */
            if (j >= nSpin)
                break;
...
#pragma HLS UNROLL
            ...
            DuplicateTrotterUnits5S<MAX_NTROT>(
                nTrot, nSpin, ctlStep, iPre, j, startStep, endStep, trotters,
                up_trotter, down_trotter, dH, h, Beta, dHTunnel, JcoupLocal,
                logRandomNumber, boardLen, constraint);
        }
    }
}
